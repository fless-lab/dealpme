...
def main():
 ...
 for x in cov.get('visible_controls',[]):
  ...
  if not cid:f.append({'severity':'P0','rule_id':'REF-CTL-001','message':f'Visible control missing ID: {x}'})
  elif cid not in ids:f.append({'severity':'P0','rule_id':'REF-CTL-002','message':f'Unknown control ID: {cid}'})
 for cid in cov.get('required_exercised',[]):
  if cid not in set(cov.get('exercised',[])):f.append({'severity':'P0','rule_id':'REF-CTL-003','message':f'Required control not exercised: {cid}'})
