...
export class SessionController {
  ...
  async logout(@CurrentPrincipal() principal: Principal, @Req() req: Request): Promise<void> {
    await this.revoke(principal.sessionId, principal, req);
  }
  ...
  async revoke(@Param("sessionId", validate(IdSchema)) sessionId: string, @CurrentPrincipal() principal: Principal, @Req() req: Request): Promise<void> {
    ...
    await this.db.transaction(async (tx) => {
      if (await this.sessionService.revoke(sessionId, principal.userId, tx)) {
        await this.audit.record({ action: "SESSION_REVOKED", actorUserId: principal.userId, subjectType: "session", subjectId: sessionId, outcome: "OK", correlationId: correlationIdOf(req) }, tx);
      }
    });
  }
  ...
  async revokeAll(@CurrentPrincipal() principal: Principal, @Req() req: Request): Promise<void> {
    await this.db.transaction(async (tx) => {
      const affected = await this.sessionService.revokeAllForUser(principal.userId, tx);
      await this.audit.record({ action: "SESSION_REVOKED", actorUserId: principal.userId, subjectType: "user", subjectId: principal.userId, outcome: "OK", correlationId: correlationIdOf(req), metadata: { all: true, affected } }, tx);
    });
  }
}
