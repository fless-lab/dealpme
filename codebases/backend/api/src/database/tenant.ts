 ...
 * current_setting('app.organisation_id') et current_setting('app.roles') ; ce sont les seules
...
export function withTenant<T>(db: CoreDb, principal: Principal | null, fn: (tx: CoreTx) => Promise<T>): Promise<T> {
  return db.transaction(async (tx) => {
    if (principal) {
      ...
      await tx.execute(sql`select set_config('app.organisation_id', ${principal.organisationId}, true), set_config('app.roles', ${principal.roles.join(",")}, true)`);
    } else {
      await tx.execute(sql`select set_config('app.organisation_id', '', true), set_config('app.roles', '', true)`);
    }
    return fn(tx);
  });
}
