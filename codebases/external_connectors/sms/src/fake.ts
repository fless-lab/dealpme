 ...
 */
export function createFakeSms(): SmsPort {
  const notImplemented = (method: string) => () =>
    Promise.reject(new Error(`Faux connecteur sms : méthode ${method} à implémenter dans le faux selon le scénario de test`));
  ...
  return port as unknown as SmsPort;
}
