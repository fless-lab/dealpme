...
export function createFakeRemo(): RemoPort {
  const notImplemented = (method: string) => () =>
    Promise.reject(new Error(`Faux connecteur remo : méthode ${method} à implémenter dans le faux selon le scénario de test`));
  ...
  return port as unknown as RemoPort;
}

export const PORT_METHODS = ["createEvent", "participantJoinUrl"] as const;
