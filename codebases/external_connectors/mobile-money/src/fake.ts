 ...
 */
export function createFakeMobileMoney(): MobileMoneyPort {
  const notImplemented = (method: string) => () =>
    Promise.reject(new Error(`Faux connecteur mobile-money : méthode ${method} à implémenter dans le faux selon le scénario de test`));
  ...
  return port as unknown as MobileMoneyPort;
}

export const PORT_METHODS = ["initiate"] as const;
