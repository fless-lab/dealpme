...
export async function POST(req:NextRequest,ctx:{params:Promise<{path:string[]}>}) {
  ...
  if(!/^events(?:\/[0-9a-f-]{36}\/(?:edit|publish|cancel|sync-attendance))?$/.test(target)&&!/^events\/diaspora\/[0-9a-f-]{36}\/decision$/.test(target)) return NextResponse.json({ok:false,message:"Action inconnue"},{status:404});
