...
export default async function OrganiserEvents(){
  ...
  return <div className="dp-stack"><div><p className="dp-kicker">Deal-Connect · organisation</p><h1>Vos événements</h1><p className="dp-muted">Préparer, publier et suivre les rencontres depuis DealPME.</p><a href="/organisateur/evenements/nouveau" className="dp-btn dp-btn-primary" data-control-id="ORG_EVENT_NEW">Créer un événement</a></div>
    ...
    <StateBanner tone="info" title={data.provider==="local"?"Intégration locale simulée":data.provider==="remo"?"Adaptateur Remo configuré":"Intégration désactivée"}>Les opérations sont pilotées depuis DealPME. La recette du compte réel confirme ses droits API, le SSO, le rendu des visuels et ses quotas.</StateBanner>
    <Panel title="Compte partagé"><p>Référence : {data.account.key}. Limite configurée {data.provider==="local"?"pour la simulation":"pour ce compte"} : <strong>{data.account.concurrentLimit} événements simultanés</strong>, avec {data.account.marginMinutes} minutes de marge de chaque côté.</p><p className="dp-muted">Ce registre couvre les réservations qui y passent. Les événements créés directement chez le fournisseur ne sont pas recensés automatiquement.</p></Panel>
