...
export default async function EventDetail({params}:{params:Promise<{eventId:string}>}){
  ...
  return <div className="dp-stack"><a href="/organisateur/evenements">← Vos événements</a><div style={{borderTop:`6px solid ${e.branding.accent}`,paddingTop:16}}><p className="dp-kicker">{e.branding.label}</p><h1>{e.title}</h1><p>{e.branding.welcome}</p></div>
    <Panel title="État et planning"><p><strong data-control-id="ORG_EVENT_STATE">{EVENT_STATE[e.status]??e.status}</strong> · {e.audience==="PUBLIC"?"Public":"Privé — Guichet Diaspora"}</p><p>{fmtDateTime(e.startsAt)} → {fmtDateTime(e.endsAt)} · {e.capacity} places</p><p className="dp-muted">Fournisseur : {e.provider}. Révision {e.revision}. Profil {e.brandingOrigin.scope} / {e.brandingOrigin.version}.</p>{e.provider==="local"?<StateBanner tone="info" title="Salle synthétique">Aucune réunion Remo réelle n'est créée. Le branding affiché est propre à cette session locale.</StateBanner>:null}{e.syncError?<StateBanner tone="warning" title="Synchronisation à rapprocher">{e.syncError}. Le créneau n'est pas libéré sur un résultat inconnu.</StateBanner>:null}{e.reservation?<p>Réservation {e.reservation.state} sur {e.reservation.accountKey}, marges incluses.</p>:<p>Aucun créneau réservé.</p>}<EventActions eventId={e.id} status={e.status} provider={e.provider}/></Panel>
    {["DRAFT","CREATE_REJECTED"].includes(e.status)&&e.audience==="PUBLIC"?<Panel title="Modifier le brouillon"><EventForm event={e}/></Panel>:null}
    {e.provider==="remo"&&e.status==="PUBLISHED"?<Panel title="Personnaliser le contenu publié dans Remo"><EventForm event={e} remoteContentOnly/></Panel>:null}
    {e.provider==="remo"&&e.status==="PUBLISHED"?<Panel title="Invitations et groupes"><ul>{e.registrations?.map(r=><li key={r.id}>{r.displayName} : {r.invitationState} — {r.providerRole==="speaker"?"intervenant":"participant"}{!r.providerConsentAt?" — accord de transmission email absent":""}</li>)}</ul><MemberGroup eventId={e.id} registrations={e.registrations??[]}/></Panel>:null}
